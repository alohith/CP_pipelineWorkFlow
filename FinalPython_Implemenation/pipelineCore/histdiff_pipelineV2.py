...
def calcHistDiffScores(
    ...
    if len(blockDef) == 0:
        ...
        undefinedBlocks = set().union(*(cleanWellNameList(x=i) for i in blockDef))
    ...
    for group in blockDef:
        select_wells = set(["".join([i[0], str(int(i[1:]))]) for i in group])
        ...
        available_vehicle_control_wells = list(
            set(vehicleCntrlWells).intersection(hd_group.index)
        )
        ...
        histdiff_df = pd.DataFrame(hds, index=featureNames, columns=hd_group.index).T
...
class CommandLine:
    def __init__(self, inOpts=None):
        ...
        self.parser.add_argument(
            ...
            default=tuple(),
            nargs="+",
            help="Defines all wells in a block (just list wells in the block)",
            type=str,
        )
        if inOpts is None:
...
def main(inOpts=None):
    ...
    blocks = (
        (cl.args.blocks,) if not isinstance(cl.args.blocks, tuple) else tuple()
    )  # need to handle longer cases
    ...
    # feats = [cleanColNames(i) for i in feats]
    # badFeats = [cleanColNames(i) for i in badFeats]
    ...
    # print(badFeats, file=sys.stderr)
    # hd = hd[[i for i in hd.columns if i not in badFeats]]
